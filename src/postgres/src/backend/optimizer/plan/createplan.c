					  ...
					  bool indexonly)
{
	 ...
	 */
	stripped_indexquals =
		!bms_is_empty(root->yb_curbatchedrelids) && IsYugaByteEnabled()
		? yb_get_actual_batched_clauses(root->yb_curbatchedrelids,
										indexquals,
										best_path)
					 ...
					 NestPath *best_path)
{
	...
	if (yb_is_batched)
	{
		 ...
		 * the equality operator to RECORD_EQ.
		 */
		ListCell *l;
		yb_hashClauseInfos =
		...
		foreach(l, joinrestrictclauses)
		{
			...
			if (rinfo->can_join &&
				...
				can_batch_rinfo(rinfo, outerrelids, inner_relids))
			{
				/* if nlhash can process this */
				Assert(is_opclause(rinfo->clause));
				RestrictInfo *batched_rinfo =
					get_batched_restrictinfo(rinfo,outerrelids,
											 inner_relids);
