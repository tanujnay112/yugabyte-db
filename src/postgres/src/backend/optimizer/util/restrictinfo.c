...
{
	int a_int = ((const ExprColSortItem*) a)->index_col;
	int b_int = ((const ExprColSortItem*) b)->index_col;
...
static Expr *zip_batched_exprs(List *b_exprs)
{
	...
	ExprColSortItem *exprcols =
		palloc(sizeof(ExprColSortItem) * list_length(b_exprs));
	...
	foreach(lc, b_exprs)
	{
		...
		int left_attno = ((Var *) get_leftop(b_expr))->varattno;
		exprcols[i].index_col = left_attno;
		exprcols[i].expr_lcell = lc;
		i++;
	}

	/* Sort based on index column. */
	qsort_arg(exprcols, list_length(b_exprs), sizeof(ExprColSortItem),
			  _exprcol_cmp, NULL);
	...
	{
		Expr *b_expr = (Expr *) lfirst(exprcols[i].expr_lcell);
...
List *
yb_get_actual_batched_clauses(Relids batchedrelids,
							  ...
							  IndexPath *inner_index)
{
	...
	foreach(l, restrictinfo_list)
	{
		...
		if (tmp_batched)
		{
			batched_list = lappend(batched_list, tmp_batched->clause);
			required_relids =
	...
	// move this somewhere that isn't a "get" method
